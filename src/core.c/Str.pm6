...
#?if !jvm
    ...
    sub samemark(Str:D: Str:D $pattern --> Str:D) {
        nqp::if(
          nqp::chars(nqp::unbox_s($pattern)),        # something to work with
          nqp::stmts(
            ...
            nqp::while(                               # for all marks
              ...
              nqp::bindpos_s($result,$i,              # store the result of:
                nqp::stmts(
                  ...
                  (
                  my $input-parts   := nqp::getattr(get-parts($input-nfd), List,'$!reified');
                  my $pattern-parts := nqp::getattr(get-parts($pattern-nfd), List, '$!reified');
